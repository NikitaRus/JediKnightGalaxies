...
void DisplayContext::DrawText( float x, float y, float scale, vec4_t color, const char *text, float adjust, int limit, int style, int iMenuFont )
{
	int iStyleOR = 0;
...
int DisplayContext::TextWidth( const char *text, float scale, int iMenuFont )
{
	int iFontIndex = MenuFontToHandle(iMenuFont);
...
int DisplayContext::TextHeight( const char *text, float scale, int iMenuFont )
{
	int iFontIndex = MenuFontToHandle(iMenuFont);
...
void DisplayContext::FillRect( float x, float y, float w, float h, const vec4_t color )
{
	SetColor( color );
...
void DisplayContext::DrawRect( float x, float y, float w, float h, float size, const vec4_t color )
{
	SetColor( color );
...
void DisplayContext::DrawSides( float x, float y, float w, float h, float size )
{
	size /= xscale;
...
void DisplayContext::DrawTopBottom( float x, float y, float w, float h, float size )
{
	size /= yscale;
...
void DisplayContext::OwnerDrawItem( itemDef_t *item, float x, float y, float w, float h, 
...
bool DisplayContext::OwnerDrawHandleKey( int ownerDraw, int flags, float *special, int key, int ownerDrawID )
{
#ifdef CGAME // fixme use _CGAME
...
void DisplayContext::RunScript( char **p )
{
...
bool DisplayContext::DeferScript( char **p )
{
#ifdef CGAME // fixme use _CGAME
	return false;
#else
...
void DisplayContext::GetTeamColor( vec4_t *color )
{
#ifdef CGAME // fixme use _CGAME
...
float DisplayContext::GetCvarValue( const char *cvar )
{
...
void DisplayContext::DrawTextWithCursor( float x, float y, float scale, vec4_t color, 
...
void DisplayContext::SetOverstrikeMode( bool b )
{
#ifndef CGAME // fixme use _CGAME
...
bool DisplayContext::GetOverstrikeMode( void )
{
#ifdef CGAME // fixme use _CGAME
	return false;
#else
...
void DisplayContext::KeyNumToStringBuf( int keynum, char *buf, int buflen )
{
#ifndef CGAME // fixme use _CGAME
...
void DisplayContext::GetBindingBuf( int keynum, char *buf, int buflen )
{
#ifndef CGAME // fixme use _CGAME
...
void DisplayContext::SetBinding( int keynum, const char *binding )
{
#ifndef CGAME // fixme use _CGAME
...
void DisplayContext::ExecuteText( int exec_when, const char *text )
{
#ifndef CGAME // fixme use _CGAME
...
void DisplayContext::Pause( bool b )
{
	if (b) {
...
int DisplayContext::PlayCinematic( const char *name, float x, float y, float w, float h )
{
	return trap_CIN_PlayCinematic(name, x, y, w, h, CIN_loop);
}

void DisplayContext::StopCinematic( int handle )
{
	trap_CIN_StopCinematic(handle);
}

void DisplayContext::DrawCinematic( int handle, float x, float y, float w, float h )
{
	trap_CIN_SetExtents(handle, x, y, w, h);
	trap_CIN_DrawCinematic(handle);
}

void DisplayContext::RunCinematicFrame( int handle )
{
	trap_CIN_RunCinematic( handle );
}

void DisplayContext::SetTime( int newRealTime )
{
	frameTime = newRealTime - realTime;
	realTime = newRealTime;
}

float DC_GetValue( int ownerDraw );
bool DC_OwnerDrawVisible( int flags );
int DC_FeederCount( float feederVal );
bool DC_FeederSelection( float feederID, int index, itemDef_t *item );
const char *DC_FeederItemText(float feederID, int index, int column,
									 qhandle_t *handle1, qhandle_t *handle2, qhandle_t *handle3);
qhandle_t DC_FeederItemImage(float feederID, int index);
int DC_OwnerDrawWidth(int ownerDraw, float scale);

void DisplayContext::RegisterCallbacks()
{
